...
public abstract class ProcessingRecipeParamsMixin {
    ...
    private static <T extends ProcessingRecipeParams> App cmr$modifyHeatCodec(App par1){
        return StringRepresentable.fromEnum(HeatCondition::values).optionalFieldOf("heat_requirement",HeatCondition.NONE).forGetter(p->((ProcessingRecipeParamsAccessor)p).cmr$getRequiredHeat());
    }
}
